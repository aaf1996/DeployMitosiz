...
function getInfoForCountDownAndPeriodo() {
    axios.post('Autocompletado.asmx/GetPeriod_Counter', {})
        .then(response => {

            let countDown = response.data.d.FechaFin;
            //let countDown = '2024-01-20T10:29:00';
            fecha = countDown;

            const namePeriodoActual = document.getElementById('namePeriodoActual');
            if (!!namePeriodoActual) {
                namePeriodoActual.innerText = response.data.d.Nombre;
            }
