...
function addRowDT(obj) {
    ...
    for (var i = 0; i < obj.length; i++) {
        ...
        var urlVoucher = "voucherCDR/" + obj[i].Voucher;
        if (obj[i].Estado == "Pendiente") { sellado = "none"; } else { sellado = ""; }
        var tipoCompra = (obj[i].TipoCompra === "10") ? "ADICIONAL - DEPOSITO" : (obj[i].TipoCompra === "13") ? "ADICIONAL - CANJE" :
            "SALDO DISPONIBLE";
        var voucher = (obj[i].TipoCompra != "3" && obj[i].Estado == "Pendiente" && obj[i].Voucher == "") ?
            '<button type="button" class="btn btn-primary btn-update" data-toggle="modal" data-target="#modalVoucher"><i class="fas fa-file-invoice-dollar"></i></button>' :
            (obj[i].Voucher != "") ? '<p> <a href="' + urlVoucher + '" target="_blank">' + obj[i].Voucher + '</a></p>':'';
        tabla.fnAddData([
            obj[i].IdSolicitud,
...
function addRowDTDetalle(obj, lblMonto) {
    ...
    for (var i = 0; i < obj.length; i++) {
        ...
        tabla3.fnAddData([
            ...
            '<img src="https://yosoymitosis.com/products/' + imagenProducto + '" style="height: 80px">',
            packs,
            obj[i].Cantidad,
...
function AgregarProducto(lstDatos) {
    ...
    if (tipoCom == "3" && (ValMonto > saldoD)) {
        ...
        d += '<tr id="tr1' + lstDatos[0].IdProducto + '">' +
            '<td rowspan="2" class="borderBottom"><img src="https://yosoymitosis.com/products/' + lstDatos[0].Foto + '" alt="Alternate Text" style="max-width: 100px !important;"/></td>' +
